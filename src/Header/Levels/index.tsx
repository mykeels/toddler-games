    ...
    <div className="flex flex-row gap-2 relative group" ref={ref}>
      ...
      >
        <img
          ...
          alt="ladder"
          className={clsx('transform duration-300 w-10 h-10', {
            'rotate-[30deg]': isOpen,
            'rotate-0': !isOpen,
          })}
        />
...
const LevelContext = createContext<{ level: number; setLevel: (level: number) => void }>({
  level: 1,
  setLevel: () => {},
});
...
export const LevelContextProvider = ({
  ...
  return (
    <LevelContext.Provider
      value={{
        level,
        setLevel: (level) => debouncedSetLevel(level),
      }}
    >
      {children}
    </LevelContext.Provider>
